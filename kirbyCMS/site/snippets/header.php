  ...
  <nav class="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8" aria-label="Global">
    <div class="flex lg:flex-1">
      <a href="#" class="-m-1.5 p-1.5">
        <span class="sr-only">Guido Offermans</span>
    ...
    <div class="hidden lg:flex lg:gap-x-12">
      ...
      <?php foreach ($site->pages()->listed() as $item) : ?>
        <a<?php e($item->isOpen(), ' class="text-sm font-semibold leading-6 text-white active"') ?>
          class="text-sm font-semibold leading-6 text-white"
          href="<?= $item->url() ?>"><?= html($item->title()) ?></a>
      <?php endforeach ?>
      ...
      <div class="mt-6 flow-root">
        <div class="-my-6 divide-y divide-gray-500/25">
          <div class="space-y-2 py-6">
            ...
            <?php foreach ($site->pages()->listed() as $item) : ?>
              <?php $class = "-mx-3 block rounded-lg py-2 px-3 text-base font-semibold leading-7 text-white hover:bg-gray-800" ?>
              <a
                <?php e($item->isOpen(), ' class="active ' . $class . '"', ' class="' . $class . '"') ?>
                href="<?= $item->url() ?>"
              >
                <?= html($item->title()) ?>
              </a>
